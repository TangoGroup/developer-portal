...
description: 'A step-by-step recipe on how to make your first call to the Completions API.'
---

This guide provides a practical, step-by-step recipe for authenticating with the Gloo AI API and making a basic call to the Chat Completions endpoint.

The process involves two main steps:
1.  **Get an Access Token:** Exchange your Client ID and Client Secret for a temporary bearer token.
...
Before you can make API calls, you need to generate a short-lived access token. You do this by sending your Client ID and Client Secret to our OAuth2 token endpoint.

<Info>
You can get your **Client ID** and **Client Secret** from the [API Credentials page](/studio/manage-api-credentials) in the Gloo AI Studio.
</Info>
...
def get_access_token():
    """
    Retrieves a new access token from the Gloo AI API.
    """
    headers = {
...
func getAccessToken() (*TokenInfo, error) {
	...
	tokenInfo.ExpiresAt = time.Now().Unix() + int64(tokenInfo.ExpiresIn)
...
public class GlooApiAuth {
    ...
    public static TokenInfo getAccessToken() throws IOException, InterruptedException {
        ...
        Gson gson = new Gson();
        TokenInfo tokenInfo = gson.fromJson(response.body(), TokenInfo.class);
    ...
    public static void main(String[] args) {
        try {
            ...
            Gson gson = new Gson();
            String jsonOutput = gson.toJson(token);
...
A successful response will contain your `access_token`, its `token_type`, and its `expires_in` duration in seconds.

---

## Step 2: Make a Chat Completion Call

Once you have a valid access token, you can call the Completions API. The examples below show a basic request.

<CodeGroup>

```bash cURL
# This example assumes you have already obtained a token
# and stored it in a variable like $ACCESS_TOKEN.

curl -X POST "https://platform.ai.gloo.com/ai/v1/chat/completions" \
